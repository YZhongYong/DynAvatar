...
class Loss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth,epoch):
        ...
        # 正则化损失
        lambda_normal = 0.05 if epoch > 60 else 0.0
        lambda_dist =  0.0
        
        rend_dist = model_outputs["rend_dist"]
        rend_normal  = model_outputs['rend_normal']
        surf_normal = model_outputs['surf_normal']
        normal_error = (1 - (rend_normal * surf_normal).sum(dim=0))[None]
        normal_loss = lambda_normal * (normal_error).mean()
        
        dist_loss = lambda_dist * (rend_dist).mean()
        
        out['loss'] += dist_loss + normal_loss
        return out